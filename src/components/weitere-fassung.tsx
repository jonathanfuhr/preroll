...
/**
 * Woher die Abweichung kommt und was an ihr abweicht.
 *
 * Ohne den Hinweis müsste der Kunde zwei Texte nebeneinanderlegen, um zu
 * sehen, ob sich überhaupt etwas geändert hat.
 */
export function FassungKopf({ fassung }: { fassung: AnzeigeFassung }) {
  const abweichung = [
...
/**
 * Die abweichende Caption einer Fassung, die ihren eigenen Rahmen hat.
 *
 * Steht **neben** dem Rahmen, auf seiner Höhe: Links sieht man, wie es
 * aussieht, rechts liest man den Text — dieselbe Aufteilung wie oben zwischen
 * Gerät und Haupt-Caption. Untereinander gestellt läge zwischen Bild und Text
 * ein halber Bildschirm.
 */
export function FassungText({ fassung }: { fassung: AnzeigeFassung }) {
  const { text, hashtags } = teileCaption(fassung.caption)

  return (
    <div>
      <FassungKopf fassung={fassung} />
...
/**
 * Eine abweichende Fassung ohne eigenen Rahmen — Instagram, Facebook.
 *
 ...
 *
 * LinkedIn ist die Ausnahme und läuft hier nicht durch: Dort *ist* der Rahmen
 * die Vorschau, und er steht links unter dem Gerät.
 */
export function WeitereFassung({ fassung }: { fassung: AnzeigeFassung }) {
  const { text, hashtags } = teileCaption(fassung.caption)
