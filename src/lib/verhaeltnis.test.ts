...
describe('Was je Typ zur Wahl steht', () => {
  it('nennt das Erste als Standard', () => {
    expect(standardVerhaeltnis('BEITRAG')).toBe('HOCH_3_4')
    expect(standardVerhaeltnis('KARUSSELL')).toBe('HOCH_3_4')
    expect(standardVerhaeltnis('REEL')).toBe('VERTIKAL_9_16')
  })

  it('lässt beim Beitrag 3:4 und 1:1 zu, sonst nichts', () => {
    expect(ERLAUBT.BEITRAG).toEqual(['HOCH_3_4', 'QUADRAT_1_1'])
